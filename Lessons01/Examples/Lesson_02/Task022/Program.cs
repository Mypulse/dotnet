...
{
    double x = (ax - bx) * (ax - bx);                          // С = (ax - bx)^ (Вычитаем координаты по X и возводим в квадрат) - Развернутый способ
    double y = Math.Pow(ay - by, 2);                           // С = (ay - by)^ (Вычитаем координаты по Y и возводим в квадрат) - Альтернативный способ
    double result = Math.Sqrt(y + x);                          // Извлечение из корня с использованием метода "Math"
...
Print(ax, ay, bx, by, dis);                                    // Выводим на печать с возможностью ввода значений и последующего решения "dis" он же "GetDistance"
